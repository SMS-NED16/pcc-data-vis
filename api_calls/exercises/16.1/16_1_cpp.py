from langData import LangData as lang
import pygal
...
#List of language names for which GitHub data will be fetched
#Langauge names are also used as identifiers for API Call URL generation
languages = ['cpp', 'c', 'python', 'matlab', 'javascript']

#Empty list to store lang objects for pygal plotting
langObjs = []

#Fetching data for all languages
for name in languages:
	temp = lang(name)
	langObjs.append(temp)

#Creating graph to compare total repos for all languages
createLangComparisonGraph(langObjs)

#Creating graphs to show highest-starred repos for each language
for langObj in langObjs:
	createReposGraph(langObj)